...
const slugify = (str) => {
  return str.normalize('NFD')
    ...
    .toLowerCase()
    .replace(/\s+/g, '-') // Khoảng trắng thành gạch nối
    .replace(/[^\w-]+/g, ''); // Xóa ký tự đặc biệt
};

// --- DỮ LIỆU CỦA BẠN (Bạn hãy paste toàn bộ libraryData từ data.js vào đây) ---
const libraryData = [
 {
...
async function main() {
  for (const item of libraryData) {
    try {
      ...
      console.log(`Tải: ${catSlug}/${ethnicSlug}/${fileName}`);
      await downloadImage(item.image, savePath);
    } catch (err) {
      console.error(`Lỗi ${item.id}:`, err.message);
    }
  }
}
main();