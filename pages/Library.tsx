...
// Import dữ liệu mẫu từ kho vừa tạo (nếu Database trống thì nó mới dùng đến)
import { libraryData } from '../data/mockData'; 

type Category = 'architecture' | 'ritual' | 'festival';

const Library: React.FC = () => {
  // Bắt đầu với mảng rỗng, dữ liệu sẽ được "kéo" từ Supabase về
  ...
  // KÉO DỮ LIỆU TỪ NƠI LƯU TRỮ (SUPABASE)
  useEffect(() => {
    const fetchLibrary = async () => {
      setIsLoading(true);
      try {
        let dbItems = await contentService.getLibraryItems();
        
        // Nếu DB chưa có gì, tự động mượn dữ liệu từ kho mockData đẩy lên
        if (!dbItems || dbItems.length === 0) {
          console.log("Đang nạp dữ liệu từ kho lưu trữ...");
          for (const item of libraryData) {
            const { id, ...insertData } = item; 
            await contentService.addLibraryItem(insertData);
          }
          dbItems = await contentService.getLibraryItems();
        }

        if (dbItems && dbItems.length > 0) {
          setItems(dbItems as LibraryItem[]);
        }
      } catch (error) {
  ...
  const availableEthnics = useMemo(() => {
    const ethnics = new Set(items.map(item => item?.ethnic || 'Khác'));
  ...
  const filteredData = useMemo(() => {
    return items.filter(item => {
      ...
      const matchSearch = 
        (item?.ethnic || '').toLowerCase().includes(searchTerm.toLowerCase()) || 
      ...
      <div className="max-w-[1920px] mx-auto px-4 lg:px-6 py-8 lg:py-12 relative z-10">
        ...
        <div className="lg:hidden mb-8 space-y-4 sticky top-20 z-30 bg-[#F7F3E9]/95 backdrop-blur-md py-2 -mx-4 px-4 border-b border-gold/10 shadow-sm">
           ...
           <div className="flex items-center gap-2 group bg-white/50 p-2 rounded-xl border border-gold/10 shadow-inner">
              <button onClick={scrollLeft} className="p-2 bg-gold/10 hover:bg-gold hover:text-white rounded-lg text-gold transition-colors shrink-0 active:scale-95 shadow-sm" aria-label="Cuộn trái"><span className="material-symbols-outlined text-lg">chevron_left</span></button>
              ...
              <button onClick={scrollRight} className="p-2 bg-gold/10 hover:bg-gold hover:text-white rounded-lg text-gold transition-colors shrink-0 active:scale-95 shadow-sm" aria-label="Cuộn phải"><span className="material-symbols-outlined text-lg">chevron_right</span></button>
           </div>
        </div>
          ...
          <main className="flex-1 w-full">
            ...
            {isLoading ? (
               <div className="text-center py-32 bg-white/50 rounded-[3rem] border-2 border-dashed border-gold/20">
                  ...
                  <p className="text-text-soft font-bold text-lg">Đang kết nối lưu trữ...</p>
               </div>
            ) : currentData.length > 0 ? (
              <>
                <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3 md:gap-6">
                  {currentData.map((item) => (
                    <div key={item?.id || Math.random().toString()} onClick={() => setSelectedItem(item)} className="group bg-white rounded-2xl md:rounded-[2rem] overflow-hidden border border-gold/15 shadow-md hover:shadow-2xl hover:-translate-y-2 active:scale-[0.98] transition-all duration-300 cursor-pointer relative flex flex-col h-[300px] md:h-[400px]">
                      <div className="h-40 md:h-56 overflow-hidden relative shrink-0 bg-[#F2EFE6]">
                        <img src={item?.image || 'https://placehold.co/400x300'} alt={item?.title || 'Di sản'} className="w-full h-full object-cover transition-transform duration-[1.5s] ease-out group-hover:scale-105" loading="lazy" onError={(e) => (e.currentTarget.src = 'https://placehold.co/400x300?text=S%E1%BA%AFc+Vi%E1%BB%87t')} />
                        <div className="absolute top-2 left-2 md:top-4 md:left-4 bg-primary/90 text-white text-[8px] md:text-[9px] font-black px-2 md:px-3 py-1 md:py-1.5 rounded-full uppercase tracking-widest backdrop-blur-md border border-white/20 shadow-md z-10">{item?.ethnic || 'Khác'}</div>
                      </div>
                      <div className="p-3 md:p-6 flex flex-col flex-grow">
                        <h3 className="text-xs md:text-lg font-black text-text-main mb-1 md:mb-2 group-hover:text-primary transition-colors line-clamp-2 leading-tight">{item?.title || 'Đang cập nhật'}</h3>
          ...
          <div className="fixed inset-0 z-[9999] flex items-center justify-center px-4 font-display">
            ...
            <div className="bg-white w-full max-w-5xl h-[85vh] md:h-auto md:max-h-[90vh] rounded-[2rem] shadow-2xl relative z-10 animate-slide-up flex flex-col md:flex-row overflow-hidden border-4 border-gold/20">
               ...
               <div className="w-full md:w-[70%] h-1/2 md:h-auto relative shrink-0 group bg-[#F2EFE6] border-b md:border-b-0 md:border-r border-gold/10">
                 <img src={selectedItem?.image || 'https://placehold.co/800x600'} alt={selectedItem?.title} className="w-full h-full object-cover" onError={(e) => (e.currentTarget.src = 'https://placehold.co/800x600?text=S%E1%BA%AFc+Vi%E1%BB%87t')} />
               ...
               <div className="w-full md:w-[30%] h-1/2 md:h-auto p-6 md:p-8 bg-white relative flex flex-col">
                 <div className="flex-1 overflow-y-auto custom-scrollbar relative z-10 animate-fade-in delay-100 pr-2">
                    <div className="flex items-center gap-3 mb-4">
                       ...
                       <span className="text-xs font-black text-primary uppercase tracking-widest">
                         {selectedItem?.category === 'architecture' ? 'Kiến trúc' : selectedItem?.category === 'ritual' ? 'Nghi lễ' : 'Lễ hội'}
                       </span>
                    </div>
                 ...
                 <div className="pt-4 border-t border-gold/10 flex gap-4 shrink-0 mt-2">
                        <button onClick={() => setSelectedItem(null)} className="flex-1 bg-primary text-white py-3 rounded-xl font-black uppercase text-[10px] tracking-widest hover:bg-gold hover:text-text-main transition-all shadow-xl">Đóng lại</button>
                 </div>
               </div>
            </div>
          </div>, document.body
